...
public interface CountryRepository extends BaseRepository<Country> {
  ...
  Page<Country> findByNameLikeOrIso2LikeOrIso3Like(@Param("name")String name, @Param("iso2")String iso2, @Param("iso3")String iso3,
      Pageable pageable);

  @Query("CALL db.index.fulltext.queryNodes('Country-Trl', $name) YIELD node "
      + " MATCH (m:Country)"
      + " WHERE m.iso2  =~ $iso2 OR m.iso3  =~ $iso3 OR id(node) = id(m) RETURN count(m)")
  long countByNameLikeOrIso2LikeOrIso3Like(@Param("name")String name, @Param("iso2")String iso2,@Param("iso3") String iso3);
  ...
  Country addIntermediateRegionToCountry(@Param("countryId")Long countryId, @Param("intermediateRegionId")Long intermediateRegionId);
  ...
  Country addSubRegionToCountry(@Param("countryId")Long countryId, @Param("subRegionId")Long subRegionId);
