...
public interface CountryRepository extends BaseRepository<Country> {
  ...
  @Query(
      value =
          "CALL db.index.fulltext.queryNodes('Country-Trl', $name) YIELD node RETURN node :#{orderBy(#pageable)} SKIP $skip LIMIT $limit",
      countQuery =
          "CALL db.index.fulltext.queryNodes('Country-Trl', $name) YIELD node RETURN count(node)")
  ...
  Page<Country> findByNameLikeOrIso2LikeOrIso3Like(
  ...
  @Query(
      "CALL db.index.fulltext.queryNodes('Country-Trl', $name) YIELD node "
          + " MATCH (m:Country)"
          + " WHERE m.iso2  =~ $iso2 OR m.iso3  =~ $iso3 OR id(node) = id(m) RETURN count(m)")
  long countByNameLikeOrIso2LikeOrIso3Like(
      @Param("name") String name, @Param("iso2") String iso2, @Param("iso3") String iso3);
  ...
  Country getById(@Param("id") UUID id);
  ...
  Country addIntermediateRegionToCountry(
      @Param("countryId") UUID countryId, @Param("intermediateRegionId") UUID intermediateRegionId);

  @Query(
      "MATCH (m:Country) WITH m MATCH (c:Region) WHERE id(m) = $countryId AND id(c) = $regionId"
          + " CREATE (m)-[r:HAS_REGION]->(c) RETURN m")
  Country addRegionToCountry(@Param("countryId") UUID countryId, @Param("regionId") UUID regionId);

  @Query(
      "MATCH (m:Country) WITH m MATCH (c:SubRegion) WHERE id(m) = $countryId AND id(c) = $subRegionId"
          + " CREATE (m)-[r:HAS_SUB_REGION]->(c) RETURN m")
  Country addSubRegionToCountry(
      @Param("countryId") UUID countryId, @Param("subRegionId") UUID subRegionId);
}